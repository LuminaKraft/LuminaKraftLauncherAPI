...
## 🔑 Configuración de CurseForge API

### Configuración de la API Key

Para que los endpoints de CurseForge funcionen correctamente, es necesario configurar la variable de entorno `CURSEFORGE_API_KEY` con una API key válida.

```bash
# En desarrollo local
export CURSEFORGE_API_KEY="tu_api_key_aquí"
```

### ⚠️ Importante: Escapar símbolos $ en Docker

Si tu API key contiene símbolos `$`, debes escaparlos con un segundo `$` cuando la configures en Docker o en un archivo docker-compose.yml, ya que Docker interpreta los símbolos `$` como variables de entorno.

**Ejemplo:**

Si tu API key es `$2a$10$26t9aaaaaaaaaaaaaaaaaaaa`:

```yaml
# En docker-compose.yml
environment:
  - CURSEFORGE_API_KEY=$$2a$$10$$26t9aaaaaaaaaaaaaaaaaaaa
```

O al ejecutar un contenedor:

```bash
docker run -e CURSEFORGE_API_KEY='$$2a$$10$$26t9aaaaaaaaaaaaaaaaaaaa' ...
```

Esta misma consideración aplica para los secretos de GitHub Actions. 